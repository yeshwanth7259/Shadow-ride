...
export default function TripMemoriesScreen({ route }) {
  ...
  // Use context group ID to make it functional with real data
  ...
  useEffect(() => {
    const memoriesRef = dbRef(db, `${groupIdPath}/memories`);
  ...
  const pickImage = useCallback(async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ['images'], // Updated from ImagePicker.MediaTypeOptions.Images for expo-image-picker compat
      allowsEditing: true,
      quality: 0.8,
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      // Convert groupIdPath to a simple string for the upload helper
      const uploadGroupId = groupIdPath.includes('groups/') ? groupIdPath.replace('groups/', '') : groupIdPath.replace('users/', 'solo_');
  ...
  }, [pendingUri, caption, groupIdPath, user]);
